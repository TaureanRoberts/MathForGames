...
public:
	...
	Protection Level : Public*/
	float Magnitude();
	/*Prototype: friend ostream& operator<<(ostream& output, Vector3 &vector)
	Description : outputs the X, Y, and Z value to the console
	Precondition : an instance of the Vector3 class
	Postcondition : outputs the X, Y, and Z value to the console
	Protection Level : Public*/
	friend ostream& operator<<(ostream& input,  Vector3 &other);
	/*Prototype: friend istream& operator<<(istream& output, Vector3 &other)
